...
twitter_data <- read.csv("C:/Users/IMI/Desktop/Text Analytics/Twitter and Reddit/Twitter_Data.csv", header = TRUE)
reddit_data <- read.csv("C:/Users/IMI/Desktop/Text Analytics/Twitter and Reddit/Reddit_Data.csv", header = TRUE)
str(twitter_data)
str(reddit_data)

library(tidyr)
sentiment_twitter <- twitter_data %>%
  unnest_tokens(word, clean_text, token = "tweets") %>%
  inner_join(get_sentiments("bing")) 


sentiment_reddit <- reddit_data %>%
  unnest_tokens(word, clean_comment, token = "tweets") %>%
  inner_join(get_sentiments("bing"))
